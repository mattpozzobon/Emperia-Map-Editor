...
MainToolBar::MainToolBar(wxWindow* parent, wxAuiManager* manager)
{
	...
	wxBitmap trade_bitmap = wxArtProvider::GetBitmap(ART_TRADE_BRUSH, wxART_TOOLBAR, icon_size);
	...
	brushes_toolbar->AddTool(PALETTE_TERRAIN_TRADEZONE_TOOL, wxEmptyString, trade_bitmap, wxNullBitmap, wxITEM_CHECK, "Trade Zone", wxEmptyString, NULL);
	brushes_toolbar->AddSeparator();
...
void MainToolBar::UpdateButtons()
{
	...
	brushes_toolbar->EnableTool(PALETTE_TERRAIN_TRADEZONE_TOOL, has_map);
...
void MainToolBar::UpdateBrushButtons()
{
	...
	if(brush) {
		...
		brushes_toolbar->ToggleTool(PALETTE_TERRAIN_TRADEZONE_TOOL, brush == g_gui.trade_brush);
		...
		brushes_toolbar->ToggleTool(PALETTE_TERRAIN_TRADEZONE_TOOL, false);
...
void MainToolBar::OnBrushesButtonClick(wxCommandEvent& event)
{
	...
	switch (event.GetId()) {
		...
		case PALETTE_TERRAIN_PVPZONE_TOOL:
			...
			break;
		case PALETTE_TERRAIN_TRADEZONE_TOOL:
